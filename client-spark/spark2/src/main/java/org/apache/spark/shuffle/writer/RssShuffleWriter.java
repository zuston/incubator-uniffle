...
public class RssShuffleWriter<K, V, C> extends ShuffleWriter<K, V> {
  ...
  private final boolean isFailureCallback2CoordinatorEnabled;
  ...
  private RssShuffleWriter(
    ...
    this.isFailureCallback2CoordinatorEnabled = RssSparkConfig.toRssConf(sparkConf)
        .get(RSS_TASK_FAILED_CALLBACK_ENABLED);
  }
  ...
  private void writeImpl(Iterator<Product2<K, V>> records) {
    ...
    while (records.hasNext()) {
      ...
      try {
        ...
        if (isFailureCallback2CoordinatorEnabled) {
          reportTaskFailure2Coordinator(e);
        }
        throw e;
      }
    }
  ...
  private void reportTaskFailure2Coordinator(RssException exception) {
    ...
    try {
      ...
      LOG.warn("Errors on callback to coordinator. Ignore this error!", e);
    } finally {
      try {
